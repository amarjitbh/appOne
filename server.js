...
app.get('/users',function(req,res){
    ...
    req.getConnection(function(err,conn){
        if(err){
            return next("connection failed");
        }
        var query = conn.query("SELECT * FROM users",function(err,row){
            if(err){
                return next("mysql Error Please check your query");
            }else{
                //res.send(row);
                res.render('users',{title:"users-list",data:row});
            }
        })
    });
});
...
app.get('/edit-user/:user_id',function(req,res,next){
    ...
    req.getConnection(function(error,conn){
        if(error){
            ...
            var query = conn.query("SELECT * FROM users WHERE id="+userId,function(err,row){
                if(err){
                    return next("there are some problem in query");
                }else{
                    res.render('edit-user',{title:"edit-User",data:row});
                }
            });
        }
    });
...
app.post('/update-users',function(req,res){
    ...
    req.getConnection(function(error,conn){
        ...
        if(error){
            ...
            var query = conn.query("UPDATE users set ? WHERE id = ? ",[data,userId], function(err, rows){
